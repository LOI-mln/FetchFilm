    ...
    <section className="py-8 relative group/carousel">
      ...
      {canScrollLeft && (
        <button
          ...
          hover:bg-black p-2 rounded-r opacity-0 group-hover/carousel:opacity-100 transition-opacity"
        >
          <svg
            className="w-8 h-8"
            fill="none"
      ...
      <div ref={scrollContainerRef} className="flex gap-4 overflow-x-auto scrollbar-hide px-4" style={{ scrollbarWidth: "none" }}>
        {/* Pour chaque film */}
        {movies.map((movie) => (
      ...
      {/* Bouton Droite */}
      {canScrollRight && (
        <button onClick={() => scroll("right")} className="absolute right-0 top-1/2 -translate-y-1/2 z-10 bg-black/80 hover:bg-black p-2 rounded-l opacity-0 group-hover:opacity-100 transition-opacity">
